...
class PersonCard_view {
    constructor() {
        ...
        this.birthDate = this.markup.querySelector("#birthDateRoot");
    ...
    populateMarkupWithData(personData) {
        const { personNumber, name, surname, birthDate, adress, idNumber, seat, luggage, airfare } = this;
        ...
        birthDate.innerText = `Date of birth: ${personData.birthDate}`
...
export class PersonCard {
    constructor(root_element, data) {
        this.controller = new PersonCard_controller(new PersonCard_model(data), new PersonCard_view(), root_element)
    }
}